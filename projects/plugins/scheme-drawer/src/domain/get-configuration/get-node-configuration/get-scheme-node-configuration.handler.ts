...
import { ConfigurationExtensions, getBorderSection, getColorSection, IConfiguration } from '../../../components/configuration';
...
export class GetSchemeNodeConfigurationHandler
  ...
  private getScheme(item: ISchemeNode): IConfiguration {
    ...
    return {
      ...
      sections: [
        this.getCommonSection(formBuilder.getForm()),
        getColorSection(formBuilder.getForm(), 'Fill', 'fill'),
        getBorderSection(formBuilder.getForm().get('border') as FormGroup, 'Border'),
      ],
    };
  }
